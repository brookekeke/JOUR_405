...
**The scatterplots and correlations show that income and percent with a bachelor’s degree look the most related to turnout. These variables show a clear upward trend. Population and percent unemployed don’t seem very connected to turnout because the points are scattered. Median age and percent white show only small or unclear relationships.**
...
**It shows that variables like education and median age appear to relate more strongly to voter turnout, while others don’t add much. The residuals look centered around zero, meaning the model's predictions aren’t consistently too high or low. This is a somewhat useful model, but a few predictors aren't really important and could be taken out**
...
**I compared all of the new models to the full one using an adjusted R-squared. None of them were much better, but the model with income and education did a little better than the others. The differences were small, which shows that voter turnout is shaped by several factors working together rather than just one or two.**

```{r}
# Model 2
model2 <- lm(turnout ~ pct_bachelors_higher + income, data = md_turnout)
summary(model2)

# Model 3
model3 <- lm(turnout ~ median_age + income, data = md_turnout)
summary(model3)
...
**I could use it in an election story to explain how things like income or education relate to turnout and which areas are more involved. It also shows where turnout is lower and where outreach might be needed. Overall, it helps turn the data into a clear story about who is voting and why.**
