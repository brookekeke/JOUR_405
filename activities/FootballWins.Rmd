...
**This is a useful model because it explains a little over half of the variation in wins, and the F-statistic shows the predictors are meaningful when considered together. Even though not every variable is individually significant, Points Scored has a visible effect. The model gives a good sense of which factors influence winning.**
...
**Points Scored has the strongest impact on wins because it has the strongest positive correlation and is significant. Teams that score more points tend to win more often. Takeaways also seem to help, but not as much. Yards Gained doesn’t look like it has much effect on wins in this data.**

## Using the Model

Without using the words "multiple regression model", describe how you could use this model to cover a team or game.

**You could use this model to point out which stats help a team win. For example, you could explain that teams that score more points or get more takeaways usually win more games, and use that to talk about how a team is doing.**
