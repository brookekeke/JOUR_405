...
**Your answer**:
Wealth distribution and housing prices are two examples of data that might not be normally distributed. Wealth distribution would be skewed because a select group of people holds a disproportionate amount of the world's total wealth. Housing prices are generally similar, but several high-end houses go for significantly more.

A reporter should recognize if there’s data that would skew the data and make it known that there is if the data is used. They should measure the median rather than the mean so that the data isn’t averaged out.
...
**Answer**:
Maryland is in the 95th percentile of total aces. 5% of all teams scored better than Maryland. This puts them above over 300 schools and shows they are a decent volleyball team. Data could be skewed because the best teams don’t necessarily have the best percentile. For example, SFU is moving to Division III and is in the 99th percentile for aces according to the data. 
...
**Answer**:
The distribution is skewed to the left because the data stretches further to the left. About 2/3rds of the data fall between the two lines of the standard deviation. 
