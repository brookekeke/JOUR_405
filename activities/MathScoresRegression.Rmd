...
**There is a weak positive relationship between FARMS percentage and math test scores. As FARMS percentage increases, math test scores only slightly increase. Thhe relationship is not strong because the data points are scattered far apart.**
...
**I don't think this model is worth using because the p-value is not statistically significant. Additionally, the R-squared value is too low. This model shows us that there is not much of a relationship between FARMS percentage and math scores.**
...
**Whitemarsh Elementary School has the largest difference between the predicted result and the actual math scores. The actual scores were 45.83 less than predicted. Rydal West School has a large difference of 38.93.**
...
**The model's results do not support the school board member's claim. There is no statistically significant relationship between FARMS and test scores. Some schools with less FARMS program score much lower than predicted while some schools with full FARMS programs score high above what the predictions. There could be other factors influencing the test scores.**

**LEDE: Free And Reduced Priced Meals have little affect on standardized math test scores, despite school board member's claims. The results of a data alaysis suggests that there may be other outside factors influencing the scores instead.**
